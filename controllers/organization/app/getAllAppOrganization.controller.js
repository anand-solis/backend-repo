...
const GetAllAppOrganizationController = async (req, res) => {
    try {
        ...
        return res.status(200).json({ organizations: organizations, status: true, error: "", message: "Organizations fetched successfully." });
    } catch (error) {
        return res.status(500).json({ organizations: null, status: false, error: `Error: ${error}`, message: "" });
    }
}
